  ...
  with freechips.rocketchip.rocket.constants.ScalarOpConstants
{
  ...
  val cmap_vaddr         = UInt(coreMaxAddrBits.W)   // Physical address computed by CMAP at decode stage
  val cmap_will_update   = Bool()                    // This instruction will update CMAP (used to set processing bit)
  val cmap_seq           = UInt(8.W)                 // Sequence number for CMAP update validation (prevents ABA problem)

  // Store Address Buffer (SAB) conflict detection (evaluated at dispatch stage)
  val sab_vaddr_valid       = Bool()                  // STA has CMAP-predicted vaddr in cmap_vaddr (for SAB write at dispatch)
  val sab_conflict          = Bool()                  // Dispatch-stage detected Store-Load address conflict via SAB
  val sab_conflict_stq_idx  = UInt(stqAddrSz.W)       // STQ index of the conflicting store (available at dispatch)
