  ...
  with rocket.HasL1HellaCacheParameters
{
 ...
 // Prioritize emptying the store queue when it is almost full
  ...
  val ldq_retry_idx = RegNext(AgePriorityEncoder((0 until numLdqEntries).map(i => {
    ...
    // SAB conflict gating: if this load has a predicted Store-Load conflict,
    // only allow retry when the conflicting store's physical address is resolved
    // (addr.valid && !addr_is_virtual) or the store entry is already deallocated.
    // This ensures the load won't execute before the store address is known.
    val sab_stq_idx = e.uop.sab_conflict_stq_idx
    val sab_ok = !e.uop.sab_conflict ||
      !stq(sab_stq_idx).valid ||
      (stq(sab_stq_idx).bits.addr.valid && !stq(sab_stq_idx).bits.addr_is_virtual)
    e.addr.valid && e.addr_is_virtual && !block && sab_ok
  }), ldq_head))
