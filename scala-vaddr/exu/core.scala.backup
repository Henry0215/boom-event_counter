...
//      ("I$ blocked",                        () => icache_blocked),
  ...
  when (startCounter) {
    ...
    event_counters.io.event_signals(1) :=  PopCount(exe_is_ld.asUInt)       //execute ld number
    event_counters.io.event_signals(2) :=  PopCount(exe_is_st.asUInt)       //execute st number
    event_counters.io.event_signals(3) :=  io.lsu.dtlb_valid_access            //valid dtlb req number
    event_counters.io.event_signals(4) :=  io.lsu.dtlb_miss_num              //dtlb miss number
    event_counters.io.event_signals(5) :=  Mux(io.lsu.perf.tlbMiss, 1.U, 0.U)  //d-tlb miss
    event_counters.io.event_signals(6) :=  io.lsu.dcache_valid_access   //valid dcache access number
    event_counters.io.event_signals(7) :=  io.lsu.dcache_nack_num   //d-cache load & store nack number
    event_counters.io.event_signals(8) :=  Mux(io.lsu.perf.acquire, 1.U, 0.U) //dcache send req to next level number
    ...
    event_counters.io.event_signals(9) :=  PopCount(com_is_ld.asUInt)       //commit ld number
    event_counters.io.event_signals(10) :=  PopCount(com_is_st.asUInt)       //commit st number
    ...
    event_counters.io.event_signals(11) :=  Mux(io.ptw.perf.l2miss, 1.U, 0.U) //L2 TLB miss
    event_counters.io.event_signals(12) :=  Mux(misalign_excpt, 1.U, 0.U)  //misalign_excpt
    event_counters.io.event_signals(13) :=  Mux(lstd_pagefault, 1.U, 0.U)  //lstd_pagefault
    // event_counters.io.event_signals(60) :=  Mux(fetch_pagefault, 1.U, 0.U)  //fetch_pagefault (not load/store related)
    event_counters.io.event_signals(14) :=  Mux(mini_exception, 1.U, 0.U)  //mini_exception
